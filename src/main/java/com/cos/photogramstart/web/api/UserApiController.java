...
public class UserApiController {
	...
	public CMRespDto<?> update(@PathVariable int id, 
			...
			BindingResult bindingResult, 	// BindingResult는 꼭 @Valid가 적혀있는 다음 파라미터에 적어야함
		...
		if(bindingResult.hasErrors()) {
			...
			// 수정이 완료되도 세션정보가 안바껴서 페이지에 안뜸
			principalDetails.setUser(userEntity);	//세션 정보 변경
			return new CMRespDto<>(1, "회원수정완료", userEntity);	// 응답시에 userEntity의 모든 getter함수가 호출되고 json 파싱하여 응답한다. 
		}			
	}
}
